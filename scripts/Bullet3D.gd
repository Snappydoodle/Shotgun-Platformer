...
func _physics_process(delta):
	...
	trail.scale = Vector2(1,1)
	trail.modulate = modulate
	trail.default_color = $Node2D/ColorRect.color
	createTrail()
...
func createTrail():
	...
	var pos = Vector2((global_position.x - main.position.x) / (main.scale.x), (global_position.y - main.position.y) / (main.scale.y))
